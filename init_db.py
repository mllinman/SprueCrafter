...
from src.backend.app_saas import app, db, User, PrinterProfile, MarketplaceItem
...
def init_database():
    ...
    with app.app_context():
        ...
        if not admin:
            ...
            admin.set_password('Detroit1977!!')
            db.session.commit()
            print("Admin password reset to: Detroit1977!!")
        
        # Add default printer profiles if none exist
        if PrinterProfile.query.count() == 0:
            print("Adding default printer profiles...")
            default_printers = [
                {
            ...
            for printer_data in default_printers:
                printer = PrinterProfile(user_id=admin.id, **printer_data)
                db.session.add(printer)
            
            db.session.commit()
            print(f"Added {len(default_printers)} default printer profiles!")
        else:
            print("Printer profiles already exist")
