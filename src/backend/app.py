...
ALLOWED_3D_FORMATS = {"stl", "obj", "fbx", "3ds", "ply", "gltf", "glb", "dae"}
...
def generate_sprue():
    ...
    if connector_type not in valid_connectors:
        return (
            jsonify(
                {"error": f'Invalid connector type. Must be one of: {", ".join(valid_connectors)}'}
            ),
            400,
        )
    ...
    sprue_file = generator.generate(
        input_path, (build_plate_x, build_plate_y, build_plate_z), connector_type=connector_type
    )
...
def photo_to_model():
    ...
    for file in files:
        if not allowed_file(file.filename, "image"):
            return (
                jsonify(
                    {"error": f"File type not allowed: {file.filename}. Must be an image format."}
                ),
                400,
            )
    ...
    if operation == "rotate":
        ...
        try:
            ...
            if any(abs(val) > 1000 for val in [x, y, z]):
                return (
                    jsonify({"error": "Translation values must be between -1000 and 1000mm"}),
                    400,
                )
        except ValueError:
...
def generate_supports():
    ...
    if mode == "automatic":
        ...
        if overhang_angle:
            try:
                ...
                if not 0 <= overhang_angle <= 90:
                    return (
                        jsonify({"error": "Overhang angle must be between 0 and 90 degrees"}),
                        400,
                    )
            except ValueError:
        ...
        output_path = generator.generate_automatic_supports(
            input_path, overhang_angle=overhang_angle, density=density
        )
    elif mode == "manual":
        ...
        output_path = generator.generate_manual_supports(input_path, support_points)
    elif mode == "estimate":
...
def get_connector_types():
    ...
    connector_types = {
        ...
        "pyramid": {"name": "Pyramid", "description": "Pyramid-shaped connectors"},
        "triangular": {"name": "Triangular", "description": "Triangular prism connectors"},
        "square": {"name": "Square", "description": "Square prism connectors"},
        "spherical": {"name": "Spherical", "description": "Spherical/beaded connectors"},
    }
...
def pro_status():
    ...
    if not api_key:
        return jsonify({"is_pro": False, "message": "No API key provided"})
